...
async def textToSpeechMultiSeparate(text, speaker_data, supported_voices):
    ...
    try:
        ...
        output_file = os.path.join(outputs_dir, "output_multispeaker.mp3")
...
# ฟังก์ชันส่งออกเสียงในรูปแบบต่างๆ
def export_audio_format(audio_format, audio_quality):
    outputs_dir = os.path.join(os.path.dirname(__file__), "outputs")
    source_file = os.path.join(outputs_dir, "output_multispeaker.mp3")
    
    if not os.path.exists(source_file):
        raise gr.Error("กรุณาสร้างเสียงก่อนดาวน์โหลด")
    ...
    try:
        ...
        ext = format_ext[audio_format]
        output_file = os.path.join(outputs_dir, f"output_audio.{ext}")
        
        # ถ้าเป็น MP3 ให้ copy โดยตรง
        if audio_format == "MP3":
            shutil.copy2(source_file, output_file)
            return output_file
        
        # ถ้าไม่มี pydub ให้ copy เป็น MP3 แทน
        if not PYDUB_AVAILABLE:
            shutil.copy2(source_file, output_file.replace(f".{ext}", ".mp3"))
            raise gr.Error(f"ต้องติดตั้ง pydub และ ffmpeg สำหรับ {audio_format} (ส่งออกเป็น MP3 แทน)")
        
        # ใช้ pydub แปลงรูปแบบ
        audio = AudioSegment.from_mp3(source_file)
        
        # กำหนด bitrate ตามคุณภาพ
        bitrate_map = {
            "High (320kbps)": "320k",
            "Medium (192kbps)": "192k", 
            "Low (128kbps)": "128k"
        ...
        if audio_format == "WAV":
            ...
            audio.export(output_file, format="aac", bitrate=bitrate)
        ...
        # ถ้าเกิด error ให้ส่งออกเป็น MP3 แทน
        mp3_file = os.path.join(outputs_dir, "output_audio.mp3")
        shutil.copy2(source_file, mp3_file)
        raise gr.Error(f"ไม่สามารถแปลงเป็น {audio_format} ได้ ส่งออกเป็น MP3 แทน: {str(e)}")
...
# ฟังก์ชันสร้างวิดีโอแบบใหม่ (สร้างแยกแล้วรวม)
async def create_video_with_images(text, speaker_data, speaker_images, supported_voices, video_format, video_quality):
    if not text.strip():
    ...
    try:
        # กำหนดขนาดตามคุณภาพ (แนวตั้งสำหรับตัวละคร)
        quality_map = {
            "HD 720p": (720, 1280),
        ...
        if video_quality not in quality_map:
            video_quality = "HD 720p"
        
        video_width, video_height = quality_map[video_quality]
        ...
        # สร้างวิดีโอแยกกันแต่ละ segment
        for i, (speaker, content) in enumerate(speaker_segments):
            if speaker not in speaker_data:
                raise gr.Error(f"ไม่พบการตั้งค่าเสียงสำหรับ {speaker}")
            
            # สร้างเสียงสำหรับ segment นี้
            settings = speaker_data[speaker]
            voice_id = supported_voices[settings['voice']]
            ...
            if not os.path.exists(temp_audio) or os.path.getsize(temp_audio) == 0:
                raise gr.Error(f"ไม่สามารถสร้างเสียงสำหรับ {speaker} ได้")
            
            # โหลดเสียงเพื่อดูความยาว
            try:
                audio_clip = AudioFileClip(temp_audio)
                duration = audio_clip.duration
                
                if duration <= 0:
                    raise gr.Error(f"ไฟล์เสียงสำหรับ {speaker} มีปัญหา")
                    
            except Exception as e:
                raise gr.Error(f"ไม่สามารถโหลดไฟล์เสียงสำหรับ {speaker}: {str(e)}")
            
            # สร้างภาพสำหรับ segment นี้
            ...
            if speaker_image:
                ...
                # ใช้พื้นหลังสีดำ
                img_clip = ColorClip(size=(video_width, video_height), color=(0, 0, 0), duration=duration)
            ...
            try:
                ...
                # ถ้าไม่สามารถรวมเสียงได้ ใช้แค่ภาพ
                video_clips.append(img_clip)
                gr.Warning(f"ไม่สามารถเพิ่มเสียงสำหรับ {speaker} ได้: {str(e)}")
            
            # ไม่ปิด audio_clip ที่นี่ เพราะยังใช้อยู่ในวิดีโอ
        
        # รวมวิดีโอทั้งหมด
        if not video_clips:
            raise gr.Error("ไม่มีวิดีโอคลิปที่สร้างได้")
            
        try:
            final_video = concatenate_videoclips(video_clips, method="compose")
        except Exception as e:
            raise gr.Error(f"ไม่สามารถรวมวิดีโอได้: {str(e)}")
        ...
        ext = format_map[video_format]
        
        # บันทึกวิดีโอ
        outputs_dir = os.path.join(os.path.dirname(__file__), "outputs")
        os.makedirs(outputs_dir, exist_ok=True)
        output_video = os.path.join(outputs_dir, f"output_video.{ext}")
        
        try:
            # บันทึกวิดีโอแบบง่าย
            final_video.write_videofile(output_video, fps=24, verbose=False, logger=None)
        except Exception as e:
            raise gr.Error(f"ไม่สามารถบันทึกวิดีโอได้: {str(e)}")
...
async def create_interface():
    ...
    with gr.Blocks(css="""
        ...
        **วิธีใช้:** ใช้ [speaker1] ข้อความ [speaker2] ข้อความ เช่น:
            ...
            with gr.Column():
                ...
                with gr.Row():
                    ...
                    video_quality = gr.Dropdown(
                        choices=["HD 720p", "Full HD 1080p", "SD 480p"],
                        value="HD 720p",
                        label="คุณภาพวิดีโอ",
                        scale=1
                    )
                
                video_btn = gr.Button("ดาวน์โหลดเป็นวิดีโอ", variant="secondary")
                
            with gr.Column():
                gr.Markdown("### การตั้งค่า Speaker (สูงสุด 10 speakers)")
                gr.Markdown("**💡 คำแนะนำ:** อัปโหลดรูปภาพขนาด 720x1280 หรือสัดส่วน 9:16 (แนวตั้ง) เพื่อผลลัพธ์ที่ดีที่สุด")
        ...
        video_btn.click(
            fn=create_video_with_images,
            inputs=[text, speaker_data_state, speaker_images_state, supported_voices_state, video_format, video_quality],
            outputs=[video_output]
        )
