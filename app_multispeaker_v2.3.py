...
def export_audio_format(audio_format, audio_quality):
    source_file = os.path.join(os.path.dirname(__file__), "output_multispeaker.mp3")
    ...
    try:
        ...
        output_file = os.path.join(os.path.dirname(__file__), f"output_audio.{ext}")
        ...
        mp3_file = os.path.join(os.path.dirname(__file__), "output_audio.mp3")
...
async def create_video_with_images(text, speaker_data, speaker_images, supported_voices, video_format, video_quality):
    ...
    try:
        ...
        # บันทึกวิดีโอ
        output_video = os.path.join(os.path.dirname(__file__), f"output_video.{ext}")
